...
export async function getPost(request: FastifyRequest, reply: FastifyReply) {
    ...
    const postQueryParamsSchema = z.object({
        ...
        return (data.onlyOwnPosts && data.userId) || (!data.onlyOwnPosts && !data.userId) || data.savedPosts;
    }, {
    ...
    try {
        ...
        const { onlyOwnPosts, userId, savedPosts } = postQueryParamsSchema.parse(request.query);
